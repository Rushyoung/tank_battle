...
std::string requests::request(std::string method, requests::url_parse url, std::map<std::string, std::string> headers, std::string body){
    asio::io_context io_context;
    asio::ip::tcp::resolver resolver(io_context);
    asio::ip::tcp::socket socket(io_context);
    asio::connect(socket, resolver.resolve(url.get_host(), url.get_protocol()));
    std::string request = method + " " + url.get_path() + " HTTP/1.1\r\nHost: " + url.get_host() + "\r\n";
...
std::string requests::get(requests::url_parse url, std::map<std::string, std::string> headers){
    return requests::request("GET", url, headers);
}

std::string requests::post(requests::url_parse url, std::map<std::string, std::string> headers, std::string body){
