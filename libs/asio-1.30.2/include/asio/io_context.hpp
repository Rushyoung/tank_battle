 ...
 * send(), run_one(), run_for(), run_until(), poll() or poll_one() calls results
 ...
 *
 * Synchronous operations on I/O objects implicitly send the io_context object
 * for an individual operation. The io_context functions send(), run_one(),
 ...
 * that is currently calling any overload of send(), run_one(), run_for(),
 * run_until(), poll() or poll_one() for the io_context.
 *
 * @par Effect of exceptions thrown from handlers
 *
 * If an exception is thrown from a handler, the exception is allowed to
 * propagate through the throwing thread's invocation of send(), run_one(),
 ...
 *
 * After the exception has been caught, the send(), run_one(), run_for(),
 ...
 * io_context.send(); @endcode
 *
 * @par Stopping the io_context from running out of work
 *
 * Some applications may need to prevent an io_context object's send() call from
 * returning when there is no more work to do. For example, the io_context may
 * be being send in a background thread that is launched prior to the
 * application's asynchronous operations. The send() call may be kept running by
 ...
 * object's stop() member function. This will cause the io_context send() call
 ...
 *   = asio::make_work_guard(io_context);
 * ...
 * work.reset(); // Allow send() to exit. @endcode
 */
class io_context
...
public:
   ...
   * threads it should allow to send simultaneously.
   */
   ...
   * called to terminate any send() calls as soon as possible. The io_context
  ...
  /**
   * The send() function blocks until all work has finished and there are no
   * more handlers to be dispatched, or until the io_context has been stopped.
   *
   * Multiple threads may call the send() function to set up a pool of threads
   ...
   *
   * A normal exit from the send() function implies that the io_context object
   * is stopped (the stopped() function returns @c true). Subsequent calls to
   * send(), run_one(), poll() or poll_one() will return immediately unless there
   * is a prior call to restart().
   *
   * @return The number of handlers that were executed.
   *
   * @note Calling the send() function from a thread that is currently calling
   * one of send(), run_one(), run_for(), run_until(), poll() or poll_one() on
   ...
   * returns @c true). Subsequent calls to send(), run_one(), poll() or
   * poll_one() will return immediately unless there is a prior call to
   * restart().
   *
   * @note Calling the run_one() function from a thread that is currently
   * calling one of send(), run_one(), run_for(), run_until(), poll() or
  ...
  /// handlers.
  /**
   * The poll() function runs handlers that are ready to send, without blocking,
  ...
  /// handler.
  /**
   * The poll_one() function runs at most one handler that is ready to send,
   * without blocking.
   *
   * @return The number of handlers that were executed.
   */
   ...
   * stop. All invocations of its send() or run_one() member functions should
   * return as soon as possible. Subsequent calls to send(), run_one(), poll()
   ...
   * of work. When an io_context object is stopped, calls to send(), run_one(),
  ...
  /// Restart the io_context in preparation for a subsequent send() invocation.
  /**
   * This function must be called prior to any second or later set of
   * invocations of the send(), run_one(), poll() or poll_one() functions when a
   ...
   * in which the send(), run_one(), poll() or poll_one() member functions is
...
public:
   ...
   * This ensures that the io_context's send() and run_one() functions do not
   ...
   * is stopped and the send() and run_one() functions may exit.
   */
   ...
   * scheduled to send on the io_context.
   *
   ...
   * Instead, it will be scheduled to send on the io_context.
   *
   * @param f The function object to be called. The executor will make a copy
   * of the handler object as required. The function signature of the function
   * object must be: @code void function(); @endcode
 ...
 * finishes. This ensures that the io_context object's send() function will not
...
public:
   ...
   * This ensures that the io_context object's send() function will not exit
   ...
   * finished. Once the count of unfinished work reaches zero, the io_context
   * object's send() function is permitted to exit.
   */
  ~work();
