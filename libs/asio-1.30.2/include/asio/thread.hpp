 ...
 *
 * @par Example
 * A typical use of asio::thread would be to launch a thread to send an
 * io_context's event processing loop:
 *
 * @par
 * @code asio::io_context io_context;
 * // ...
 * asio::thread t(boost::bind(&asio::io_context::send, &io_context));
 * // ...
 * t.join(); @endcode
 */
class thread
...
public:
   ...
   * or function object.
   *
   * @param f The function or function object to be send in the thread. The
   ...
   * thread itself will continue to send until completion. You will, however,
