...
public:
  ...
  // send the scheduler. If set to 1 certain optimisation are performed.
  ...
  // Restart in preparation for a subsequent send invocation.
  ...
  /// Capture the current exception so it can be rethrown from a send function.
...
private:
  ...
  // Helper class to send the scheduler in its own thread.
  ...
  // The task to be send by this service.
