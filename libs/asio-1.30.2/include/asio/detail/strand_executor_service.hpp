...
public:
  ...
  private:
    ...
    // The handlers that are waiting on the strand but should not be send until
    // after the next time the strand is scheduled. This queue must only be
    // modified while the mutex is locked.
    op_queue<scheduler_operation> waiting_queue_;

    // The handlers that are ready to be send. Logically speaking, these are the
...
private:
  ...
  // Invokes all ready-to-send handlers.
